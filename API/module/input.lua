...
function input.getControlString(control, player)
	if type(control) ~= "string" then typeCheckError("input.getControlString", 1, "control", "string", control) end
	if player ~= nil and typeOf(player) ~= "PlayerInstance" then typeCheckError("input.getControlString", 2, "player", "PlayerInstance or nil", player) end
	local tc = keybindIDs[string.lower(control)]
	if not tc then error("'" .. control .. "' is not a known control", 2) end
